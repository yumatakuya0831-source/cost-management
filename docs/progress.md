...
1. 商品・材料・仕入価格・公開/秘密レシピの検証用データを登録
2. 原価計算・月次スナップショットの数値試験を実施
...
`docs/requirements.md`と`docs/design.md`文書版1.0を正とする。Google認証、DB、各マスタ、レシピ、月次販売数、DB原価再計算、実データダッシュボード、ユーザー管理まで実装済み。次は検証用データを登録し、原価計算と権限の実動作試験を行う。Public Gitには実ユーザー一覧を含めない。
