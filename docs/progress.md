...
| 機能実装 | 進行中 | 商品・購入先・材料・仕入価格・レシピ管理、許可ユーザー向け商品閲覧を実装 |
...
1. 月次販売個数入力と原価再計算を実装
2. ダッシュボードをSupabase実データへ接続
3. ユーザー管理を実装
4. 商品・材料・仕入価格・公開/秘密レシピの検証用データを登録
...
`docs/requirements.md`と`docs/design.md`文書版1.0を正とする。Google認証、DB、各マスタ、レシピ編集、安全な公開レシピ取得まで実装済み。次は月次販売数・原価再計算と実データダッシュボードを実装する。秘密レシピの一般ユーザー実動作試験はデータ準備後に必ず行う。
