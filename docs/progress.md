...
| 機能実装 | 進行中 | 各マスタ・レシピ・商品閲覧・月次販売個数・原価再計算を実装 |
...
1. ダッシュボードをSupabase実データへ接続
2. ユーザー管理を実装
3. 商品・材料・仕入価格・公開/秘密レシピの検証用データを登録
4. 原価計算・月次スナップショットの数値試験を実施
...
`docs/requirements.md`と`docs/design.md`文書版1.0を正とする。Google認証、DB、各マスタ、レシピ、月次販売数、DB原価再計算まで実装済み。次はダッシュボードを`get_monthly_dashboard`へ接続する。秘密レシピ・原価計算の実動作試験は検証データ準備後に必ず行う。
