...
`docs/requirements.md`と`docs/design.md`文書版1.0を正とする。検証用データはSupabaseへ登録済みで、名称先頭の`【検証】`で識別できる。原価再計算は`product_id is ambiguous`で失敗したため、0007適用後に再試験する。Public Gitには実ユーザー一覧や実レシピを含めない。
